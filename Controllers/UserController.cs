    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddUser([FromBody] User userObj)
        {
            ...
            if (await CheckEmailExistAsync(userObj.Email))
                return BadRequest(new { Message = "E-mail já existente" +
                    "" });

            //check username
            if (await CheckUsernameExistAsync(userObj.Username))
                return BadRequest(new { Message = "Este apelido já existe" });
        ...
        private static string CheckPasswordStrength(string pass)
        {
            ...
            if (pass.Length < 9)
                sb.Append("O tamanho mínimo de senha deve ser 8" + Environment.NewLine);
        ...
        private string CreateJwt(User user)
        {
            ...
            var key = Encoding.ASCII.GetBytes("veryverysceret.....veryverysceret.....");
            ...
            var tokenDescriptor = new SecurityTokenDescriptor
            {
                ...
                //Expires = DateTime.UtcNow.AddMinutes(-5).AddSeconds(10),
        ...
        public async Task<IActionResult> Refresh([FromBody]TokenApiDto tokenApiDto)
        {
            ...
            if (user is null || user.RefreshToken != refreshToken || user.RefreshTokenExpiryTime <= DateTime.Now)
                return BadRequest("Solicitção Inválida");
